...
|:------|:------------------|----:|:------------------------------|
| Input | Cust_General.csv | 10 columns | General customer info |
| Input | IC_Corporate.csv | 7 columns | Impact Card Corporates |
| Input | IC_ProjFin.csv | 6 columns | Impact Card Proj. Fin. |
| Input | IC_FinInst.csv | 22 columns | Impact Card Fin. Institutions |
| Input | IC_PrivEq.csv | 22 columns | Impact Card Priv. Equity Funds |
| Input | PEF_Investments.csv | 5 columns | Priv.Eq. Investments at review |
| Input | PEF_RegionPerc.csv | 3 columns | Priv.Eq. Asset% at contract |
| Input | Fctr_EcEmis1.csv | 23/19 | Economic Emission Factors Scope 1 |
| Input | Fctr_EcEmis2.csv | 23/19 | Economic Emission Factors Scope 2 |
| Input | Fctr_CapInt.csv | 23/19 | Capital Intensity Factors |
| Input | Fctr_CBbrkdwn.csv | 23/17 | Central Bank Breakdown factors |
| Input | Fctr_Out2Elec.csv | 23/17 | Output to Electricity |
| Input | PEFConsEC1fctrs.csv | 20 columns | Consolidated over EcEF1 |
| Input | PEFConsEC2fctrs.csv | 20 columns | Consolidated over EcEF2 |
| Input | PEFConsPKfctrs.csv | 20 columns | Consolidated over CpIF |
| Input | PEFConsSctrs.csv | 20 columns | Consolidated over sectors |
| Input | PEFConsSize.csv | 20 columns | Consolidated over MSME/Corp |
| LUT | MAP_CntrReg.csv | 127/2 | Map Client-country -> GHG-Region |
| LUT | MAP_SctrMdl.csv | 21/2 | Map Client-sector -> GHG-Sector |
| LUT | MAP_ICSctrMdl.csv | 18/2 | Map Client-sector -> IC-Sector |
| Output | KPI_aGHG.csv | 7 columns | Yearly GHG emissions per customer|

***

```{r setup_env, echo=FALSE, warning=FALSE, include=FALSE}
## Load packages used
...
source("microservices/uS009_AbsEms_Un.R")
...
tot <- round(sum(AE_PrFin2$abs_ems, na.rm = TRUE)/10^3, 0)
```
Absolute emissions: `r tot` (MtCO2e/yr)
...
tot <- round(sum(AE_PrEq1$abs_ems, na.rm = TRUE)/10^3, 0)
```

Absolute emissions total: `r tot` (MtCO2e/yr)
...
PEFConsECF <- read.csv2(fname_in_PEFConsEC2fctrs, 
                           stringsAsFactors = FALSE, fileEncoding = "UTF-8",
                           check.names = FALSE)
...
tot <- round(sum(AE_PrEq2$abs_ems, na.rm = TRUE)/10^3, 0)
```
...
tot <- round(sum(AE_FI1$abs_ems, na.rm = TRUE)/10^3, 0)
```
...
tot <- round(sum(AE_FI2$abs_ems, na.rm = TRUE)/10^3, 0)
```

Absolute emissions total: `r tot` (MtCO2e/yr)


### Client of unknown type (scope 1)

Calculate absolute emissions (scope 1) based on no data for each client of unknown type.

```{r 8.AbsEms-Un1, echo=FALSE}
custG <- read.csv2(fname_in_custG, stringsAsFactors = FALSE,
                   fileEncoding = "UTF-8", check.names = FALSE)
custG <- custG[custG$IC_Type == "unknown",]
EcEF <- read.csv2(fname_in_EcEF1, stringsAsFactors = FALSE,
                                  fileEncoding = "UTF-8", check.names = FALSE)
CpIF <- read.csv2(fname_in_CpIF, stringsAsFactors = FALSE,
                                 fileEncoding = "UTF-8", check.names = FALSE)
...
AE_Un1 <- uS009_AbsEms_Un(customers = custG,
                          ecfactors = EcEF,
                          pkfactors = CpIF,
                          countrymap = lutC,
                          sectormap = lutS)

tot <- round(sum(AE_Un1$abs_ems, na.rm = TRUE)/10^3, 0)
```

Absolute emissions total: `r tot` (MtCO2e/yr)


### Client of unknown type (scope 2)

Calculate absolute emissions (scope 2) based on no data for each client of unknown type.

```{r 8.AbsEms-Un2, echo=FALSE}
EcEF <- read.csv2(fname_in_EcEF2, stringsAsFactors = FALSE,
                                  fileEncoding = "UTF-8", check.names = FALSE)

AE_Un2 <- uS009_AbsEms_Un(customers = custG,
                          ecfactors = EcEF,
                          pkfactors = CpIF,
                          countrymap = lutC,
                          sectormap = lutS)

tot <- round(sum(AE_Un2$abs_ems, na.rm = TRUE)/10^3, 0)
```
...
AE_Corp <- bind_cols(AE_Corp1, AE_Corp2)
AE_Corp <- mutate(AE_Corp, Type = "corporate")
AE_FI <- bind_cols(AE_FI1, AE_FI2)
AE_FI <- mutate(AE_FI, Type = "financial institution")
AE_PrFin <- bind_cols(AE_PrFin1, AE_PrFin2)
AE_PrFin <- mutate(AE_PrFin, Type = "project finance")
AE_PrEq <- bind_cols(AE_PrEq1, AE_PrEq2)
AE_PrEq <- mutate(AE_PrEq, Type = "private equity fund")
AE_Un <- bind_cols(AE_Un1, AE_Un2)
AE_Un <- mutate(AE_Un, Type = "unknown")
AE <- bind_rows(AE_Corp, AE_PrFin, AE_PrEq, AE_FI, AE_Un)
AE <- select(AE, c(1,2,5,6,7))
colnames(AE)[1] <- "Customer_ID"
colnames(AE)[2] <- "Scope_1"
colnames(AE)[3] <- "Scope_2"
colnames(AE)[4] <- "Method"
colnames(AE)[5] <- "Type"
AE <- arrange(AE, Customer_ID)
rm(list=ls(pattern="AE_"))
...
tot <- round(sum(AEsum$AbsEms, na.rm = TRUE)/10^3,0)

AEsum
```

Total amount of Absolute GHG Emissions is `r tot` (MtCO2e/yr). This includes scopes 1 and 2.
